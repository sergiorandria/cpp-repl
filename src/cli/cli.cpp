...
static std::string extractValue(const std::string &arg, const std::string &prefix, int argc, char **argv, int &i, std::string &err) {
  if (arg == prefix) {
    if (i + 1 >= argc) { err = prefix + " requires argument"; return ""; }
    return argv[++i];
  }
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "-h" || arg == "--help")
      ...
      if (arg == "--include" || arg == "--include-path") {
        if (i + 1 >= argc) { err = arg + " requires argument"; return opts; }
        v = argv[++i];
      } else if (arg.rfind("--include=", 0) == 0) {
        v = arg.substr(std::string("--include=").size());
      } else {
        err = "unknown option: " + arg; return opts;
      }
      opts.includePaths.push_back(v);
      ...
      for (++i; i < argc; ++i) opts.files.push_back(argv[i]);
      break;
