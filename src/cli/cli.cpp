...
Options parse(int argc, char **argv, std::string &err) {
  ...
  for (int i=1;i<argc;++i) {
    ...
    if (arg=="-h" || arg=="--help") opts.showHelp = true;
    else if (arg=="-v" || arg=="--version") opts.showVersion = true;
    else if (arg=="--scaffold") opts.showScaffold = true;
    else if (arg=="--no-scaffold") opts.showScaffold = false;
    else if (arg=="--no-interactive") opts.noInteractive = true;
    else if (arg=="-e") {
