...
bool Sandbox::containsDangerous(const std::string &code, std::string &reason) const {
  ...
  if (!cfg_.allowSystemCalls) {
    const std::vector<std::pair<std::string, std::string>> denylist = {
        {"system", "system()"}, {"popen", "popen()"}, {"fork", "fork()"}, {"exec", "exec*()"},
        {"execl", "execl()"},   {"execv", "execv()"}, {"kill", "kill()"},
    };
  ...
  if (!cfg_.allowFileWrite) {
    const std::vector<std::string> denylist = {"unlink", "remove",  "rename",
                                               "fopen",  "freopen", "truncate"};
    for (auto &w : denylist) {
      ...
      if (std::regex_search(stripped, re)) {
        ...
        if (w == "fopen") {
          ...
          if (stripped.find("\"w") != std::string::npos ||
              stripped.find("\"a") != std::string::npos) {
  ...
  if (!cfg_.allowNetwork) {
    if (containsWord(stripped, "socket") || containsWord(stripped, "connect") ||
        containsWord(stripped, "bind") || containsWord(stripped, "listen")) {
  ...
  if (stripped.size() > cfg_.maxCodeSize) {
    reason = "code size " + std::to_string(stripped.size()) + " exceeds limit " +
             std::to_string(cfg_.maxCodeSize);
    return true;
  }
  return false;
}
