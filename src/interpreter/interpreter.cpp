...
bool Interpreter::init(utils::StdVersion version, std::string &err) {
  ...
  std::vector<const char *> args = {utils::VersionDetector::toFlag(version).c_str(), "-O0",
                                    "-resource-dir", "/usr/lib/clang/22"};
  // Need to keep string storage alive – use static or member. For now use persistent.
  // To avoid c_str() dangling, we duplicate via builder's internal copy? Actually builder stores pointers,
  // so we need to ensure c_str() lives. Use static string.
  ...
  if (!interpOrErr) {
    llvm::handleAllErrors(interpOrErr.takeError(), [&](llvm::ErrorInfoBase &EIB){ err = EIB.message(); });
    return false;
  }
...
bool Interpreter::ensureVersion(utils::StdVersion needed, std::string &err) {
  if (!initialized_) return init(needed, err);
  if (static_cast<int>(needed) <= static_cast<int>(currentVersion_)) return true;
  ...
  initialized_ = false;
  history_.clear();
  if (!init(needed, local)) { err = local; return false; }
...
bool Interpreter::evalAuto(const std::string &code, std::string &err) {
  ...
  if (!ensureVersion(needed, err)) return false;
...
bool Interpreter::eval(const std::string &code, std::string &err) {
  ...
  if (static_cast<int>(needed) > static_cast<int>(currentVersion_)) {
    std::string vErr;
    if (!ensureVersion(needed, vErr)) { err = vErr; return false; }
  }
  ...
  if (e) {
    ...
    if (needsSemi) {
      clang::Value V2; auto e2 = interp_->ParseAndExecute(code, &V2);
      if (!e2) { if (V2.isValid()) { V2.dump(); std::cout << "\n"; } if (!code.empty()) history_.push_back(code); return true; }
    ...
    if (!needsSemi && !trimmed.empty() && trimmed.back() != ';' && trimmed.back() != '}' && trimmed.back() != '{') {
      ...
      clang::Value V2; auto e2 = interp_->ParseAndExecute(withSemi, &V2);
      if (!e2) { if (V2.isValid()) { V2.dump(); std::cout << "\n"; } if (!code.empty()) history_.push_back(code); return true; }
      llvm::handleAllErrors(std::move(e2), [&](llvm::ErrorInfoBase &EIB){});
    }
    err = msg;
  ...
  if (V.isValid()) {
    ...
    if (!t.empty() && t.back() == ';') {
      ...
      if (t.find("int ") != std::string::npos || t.find("auto ") != std::string::npos ||
          ...
          t.find("std::cout") != std::string::npos || t.find("printf") != std::string::npos) likelyExpr = false;
...
bool Interpreter::eval(const std::string &code, std::string &err, bool &incomplete) {
  ...
  if (braces>0 || parens>0 || brackets>0) { incomplete=true; return true; }
  return eval(code, err);
}
...
bool Interpreter::undo(unsigned n, std::string &err) {
  ...
  while (n-- >0 && !history_.empty()) history_.pop_back();
  return true;
}
void Interpreter::dump() const {
  std::cout << "=== REPL history (" << history_.size() << " inputs) [" << utils::VersionDetector::toString(currentVersion_) << "] ===\n";
  for (size_t i=0;i<history_.size();++i) {
    std::cout << "[" << i << "] " << history_[i];
    if (history_[i].empty() || history_[i].back() != '\n') std::cout << "\n";
    std::cout << "---\n";
  }
  if (history_.empty()) std::cout << "(no inputs yet)\n";
}
void Interpreter::reset(std::string &err) {
  std::string local;
  interp_.reset(); initialized_=false; history_.clear();
  if (!init(currentVersion_, local)) err=local; else err.clear();
}
void Interpreter::help() const {
  std::cout << "C++ REPL (LLVM VM, O0, no optimizations) [" << utils::VersionDetector::toString(currentVersion_) << "]\n"
               "Commands:\n"
               ...
               "  cpp> add(2,3)\n"
               "BigInt: cpp_int / bigint via boost::multiprecision (e.g. cpp_int a = cpp_int(\"12345678901234567890\"); a*a)\n"
               "C++20/23: auto-detects 'concept', 'requires', 'import' etc. and switches to -std=c++20/23\n"
               "\n"
