...
static const std::unordered_set<std::string> kTypes = {
  "int","float","double","char","bool","void","long","short","unsigned","signed","size_t","auto","std::string","string","std::vector","std::map","cpp_int","bigint","mpz_int","int64_t","uint64_t"
};
...
std::string Highlighter::highlight(const std::string &code, bool useColor) {
  if (!useColor || code.empty()) return code;
  ...
  while (i<n) {
    ...
    if (std::isdigit((unsigned char)c) || (c=='.' && std::isdigit((unsigned char)nxt))) {
      size_t j=i;
      while (j<n && (std::isalnum((unsigned char)code[j]) || code[j]=='.' || code[j]=='_' )) ++j;
      out += YELLOW + code.substr(i, j-i) + RST;
      i=j; atLineStart=false; continue;
    }
    // word
    if (std::isalpha((unsigned char)c) || c=='_' || c==':') {
      size_t j=i;
      while (j<n && (std::isalnum((unsigned char)code[j]) || code[j]=='_' || code[j]==':')) ++j;
...
std::string Highlighter::highlightValue(const std::string &valStr, bool useColor) {
  ...
  // numeric?
  bool isNum = true;
  for (char c: valStr) if (!std::isdigit((unsigned char)c) && c!='.' && c!='-' && c!='e' && c!='E' && c!='+' && c!='f' && c!='L') { isNum=false; break; }
  if (isNum) return std::string("\033[33m") + valStr + "\033[0m";
  return std::string("\033[32m") + valStr + "\033[0m";
}
