...
bool IncompleteDetector::isIncomplete(const std::string &buffer) {
  if (hasUnclosedBrace(buffer)) return true;
  ...
  }
  {
    ...
    if (posTpl != std::string::npos) {
      ...
      if (!hasTermAfter) {
        static const std::regex reTplHeader(R"(.*\btemplate\s*(<[^>]*>)?\s*$)", std::regex::ECMAScript);
        std::smatch m;
        if (std::regex_match(t, m, reTplHeader)) return true;
        std::string afterTpl = t.substr(posTpl);
        if (afterTpl.find(';') == std::string::npos && afterTpl.find('{') == std::string::npos) return true;
        if (afterTpl.find('(') != std::string::npos && afterTpl.find(';') == std::string::npos && afterTpl.find('{') == std::string::npos) return true;
      }
    }
  }
  {
  ...
  }
  {
    ...
    if (std::regex_match(t, reRequires)) return true;
  }
  {
